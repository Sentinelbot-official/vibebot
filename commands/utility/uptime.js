...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const getUptimeQuality = (days) => {
      if (days >= 30) return { emoji: '🟢', text: 'Excellent', color: 0x00ff00 };
    ...
    if (message.client.shard) {
      ...
      try {
        ...
        const shardUptimes = await message.client.shard.fetchClientValues('uptime');
        const avgUptime = shardUptimes.reduce((a, b) => a + b, 0) / shardUptimes.length;
